...
export const AddTask = ({ taskToUpdate, onAddEdit, onCloseClick }) => {
  ...
  const [tagsInput, setTagsInput] = useState(
    taskToUpdate && taskToUpdate.tags.length > 0 ? taskToUpdate.tags.join(", ") : "",
  )
  ...
  const validateForm = () => {
    ...
    !task.title && (newErrors.title = "title is required")
    !task.description && (newErrors.description = "description is required")
    !task.priority && (newErrors.priority = "priority is required")

    const tagsArray = tagsInput.split(",").map((tag) => tag.trim())
    const hasEmptyTag = tagsArray.some((tag) => tag.length === 0)

    if (hasEmptyTag || task.tags.length === 0) {
      newErrors.tags = "at least one tag is required (no empty tags allowed)"
    }
