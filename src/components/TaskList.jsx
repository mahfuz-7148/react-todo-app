      ...
      <table className="w-full table-fixed">
        ...
        {tasks.map((task, index) => (
          <tr key={task.id} className="group border-b border-gray-700/30 hover:bg-gray-800/30 transition-all duration-200 [&>td]:align-baseline [&>td]:px-4 [&>td]:py-4">
            <td>
              <button onClick={() => onFav(task.id)} className="transition-all duration-3000 hover:scale-125 active:scale-95">
                {task.isFavorite ? (
                ...
                {task.tags.map((tag) => (
                  <li key={tag}>
                    <span
                      ...
                      className="inline-flex items-center h-6 px-3 rounded-full border text-xs font-medium capitalize backdrop-blur-sm transition-all duration-200 hover:scale-105"
                    >
                      {tag}
                    </span>
                  </li>
